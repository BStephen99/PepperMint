...
def merge_groundtruth_and_predictions(df_groundtruth, df_predictions):
  ...
  print("groundtruth", df_groundtruth["uid"].count())
...
def calculate_precision_recall(df_merged, positiveLabelsWASD):
  ...
  path = "./results/results_features_WASD.csv"
  full_path = os.path.abspath(path)

  print(f"Saving results to {full_path}")
  df_merged.to_csv("./results/results_features_WASD.csv")

  return np.array(df_merged["precision"]), np.array(df_merged["recall"])


def run_evaluation_asd(predictions, groundtruth, positiveLabelsWASD):
  ...
  precision, recall = calculate_precision_recall(df_merged, positiveLabelsWASD)
...
def get_eval_score(cfg, preds):
    ...
    if eval_type == 'AVA_ASD':
        ...
        score = run_evaluation_asd(preds, groundtruth, cfg["positiveLabelsWASD"])
