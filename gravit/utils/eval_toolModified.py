...
def get_all_positives(df_merged):
  ...
  return df_merged[df_merged["label_groundtruth"] ==
                   "SPEAKING_AUDIBLE"]["uid"].count() #+ df_merged[df_merged["label_groundtruth"] ==
                                                   #"SPEAKING_NOT_AUDIBLE"]["uid"].count()
  #return df_merged[df_merged["label_groundtruth"] ==
    #               "speaking"]["uid"].count() + df_merged[df_merged["label_groundtruth"] ==
    #                                "byplay"]["uid"].count()
...
def calculate_precision_recall(df_merged):
  ...
  df_merged["is_tp"] = np.where(
      ...
      #(df_merged["label_groundtruth"].isin(["speaking", "byplay"])))
