...
def load_csv(filename, column_names):
  ...
  #df = df[df["label"]!="SPEAKING_NOT_AUDIBLE"]
  df = df[~df["entity_id"].str.contains("pepper")]
  # Creates a unique id from frame timestamp and entity id.
  # Format values in the specified row to two decimal places
  #df["frame_timestamp"] = df["frame_timestamp"].apply(lambda x: f"{x:.2f}")
...
def get_all_positives(df_merged):
  ...
  #1-"SPEAKING_AUDIBLE"
  #2- "SPEAKING_AUDIBLE" and "SPEAKING_NOT_AUDIBLE"
  #3- "SPEAKING_NOT_AUDIBLE"
  testClass = 4
  if testClass == 1:
      return df_merged[df_merged["label_groundtruth"] == "SPEAKING_AUDIBLE"]["uid"].count()
  elif testClass == 2:
      ...
      return df_merged[df_merged["label_groundtruth"] == "SPEAKING_NOT_AUDIBLE"]["uid"].count()
  elif testClass == 4:
...
def calculate_precision_recall(df_merged):
  ...
  df_merged["is_tp"] = np.where(
      ...
      #(df_merged["label_groundtruth"] == "SPEAKING_NOT_AUDIBLE") &
  ...
  print("saving*****************************************")
  #df_merged.to_csv("/home2/bstephenson/GraVi-T/results/SPELL_ASD_default/df_merged.csv")
  #df_coords = pd.read_csv("/home2/bstephenson/GraVi-T/avaAllaugmented.csv")
  df_merged.to_csv("/home2/bstephenson/GraVi-T/results/df_merged.csv")
...
def run_evaluation_asd(predictions, groundtruth):
  ...
  #df_groundtruth = df_groundtruth.drop(df_groundtruth.index[0])
  #print(df_groundtruth.index)
  print("gt uid", df_groundtruth["uid"].shape)
  df_predictions = pd.DataFrame(predictions, columns=column_names+["score"]+["landmarks", "num_speakers"])
  print("gt uid", df_groundtruth["uid"].head(3))

  # Creates a unique id from frame timestamp and entity id.
  print(df_predictions.head(3))
  df_predictions["uid"] = (df_predictions["frame_timestamp"].map(str) + ":" + df_predictions["entity_id"])
  print("pred shape", df_predictions["uid"].head(2))
  print("uid", df_predictions["uid"].shape)
  merge_keys = ['uid']
  duplicates = df_predictions[df_predictions.duplicated(subset=merge_keys, keep=False)]
  print("duplicates", duplicates.shape)
  df_predictions = df_predictions.drop_duplicates(subset=merge_keys)
  print("Duplicates in the right dataframe (df_predictions):")
  print(duplicates['video_id'].unique())
...
def get_eval_score(cfg, preds):
    ...
    if eval_type == 'AVA_ASD':
        ...
        #groundtruth = '/home2/bstephenson/ASDNet/avaStyleCSV.csv'
        #groundtruth = '/home2/bstephenson/ASDNet/220929_CLIP_02avaStyleCSV.csv'
        groundtruth = '/home2/bstephenson/ASDNet/ava220927.csv'
        #groundtruth = '/home2/bstephenson/ASDNet/ava220926downsample.csv'
