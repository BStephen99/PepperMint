...
def merge_groundtruth_and_predictions(df_groundtruth, df_predictions):
  ...
  #print(non_unique_uids.sample(5))
  missing_in_predictions = df_groundtruth[~df_groundtruth['uid'].isin(df_predictions['uid'])]
  #print("missing_in_predictions", missing_in_predictions.shape)
  ...
  df_merged["bounding_box_correct"] = np.where(
      eq(df_merged["entity_box_x1_back_groundtruth"],
         df_merged["entity_box_x1_back_prediction"])
      & eq(df_merged["entity_box_x2_back_groundtruth"],
           df_merged["entity_box_x2_back_prediction"])
      & eq(df_merged["entity_box_y1_back_groundtruth"],
           df_merged["entity_box_y1_back_prediction"])
      & eq(df_merged["entity_box_y2_back_groundtruth"],
...
def run_evaluation_asd(predictions, groundtruth, cfg):
  ...
  df_groundtruth = load_csv(groundtruth, column_names=column_names+["landmarks_back", "set", "annotations", "label_id"], cfg=cfg)
