...
class SPELLBYPLAYGAZE2VIEWS(Module):
    def __init__(self, cfg):
        super(SPELLBYPLAYGAZE2VIEWS, self).__init__()
    ...
    def forward(self, x, edge_index, edge_attr, xH=None, c=None, cH=None, ps=None, pers=None, gender=None, gaze=None, landmarks=None, landmarksH=None, dinoEmb=None,speakerEmb=None, numPredSpeakers=None):
        ...
        if self.use_spf:
            ...
            x_visual = self.layer011(torch.cat((x[:, feature_dim//self.num_modality:], xH[:, feature_dim//self.num_modality:], self.layer_gaze(gaze), self.layer_spf(c)), dim=1))
            ...
            #x_visual = self.layer011(torch.cat((self.layer_gaze(gaze), ps, pers, self.layer_spf(c)), dim=1))
