import torch
from torch.nn import Module, ModuleList, Conv1d, Sequential, ReLU, Dropout, Softmax
...
class SPELL(Module):
    def __init__(self, cfg):
        ...
        if self.use_spf:
            ...
            self.layer_spfAudio = Linear(-1, cfg['proj_dim']) # projection layer for spatial features
            self.layer_speakerEmb = Linear(-1, 2) #6

            self.embeddings = Embedding(3, 64) #50 #20
            self.landmarkProj = Linear(-1, 100)
            self.guessGender = Linear(-1, 3)
            self.layer_ps = Linear(-1, cfg['proj_dim']) # projection pepper speaking
            #self.layer_pers = Linear(-1, cfg['proj_dim']) # projection person speaking
    ...
    def forward(self, x, edge_index, edge_attr, c=None, ps=None, pers=None, gender=None, gaze=None, landmarks=None, dinoEmb=None,speakerEmb=None, numPredSpeakers=None):
        ...
        #print(gaze)
        gender = self.embeddings(gender).squeeze(1)
        numPredSpeakers = numPredSpeakers.unsqueeze(1)
        #print(self.landmarkProj(landmarks).shape)
        #print(c)

        if self.use_spf:
            #x_visual = self.layer011(torch.cat((x[:, :feature_dim//self.num_modality], self.layer_spf(c), self.layer_ps(ps)), dim=1))
            x_visual = self.layer011(torch.cat((x[:, feature_dim//self.num_modality:],  self.layer_spf(c), gaze), dim=1)) #self.landmarkProj(numPredSpeakers))
            #x_visual = self.layer011(x[:, :feature_dim//self.num_modality]) #self.layer_spf(c), self.layer_pers(pers)
        ...
        if self.num_modality == 1:
            ...
            #x_audio = self.layer012(x[:, :feature_dim//self.num_modality])
            #x_audio = self.layer012(torch.cat((x[:, :feature_dim//self.num_modality], self.layer_spfAudio(c), self.layer_speakerEmb(speakerEmb), ps, gender), dim=1))
            x_audio = self.layer012(torch.cat((x[:, :feature_dim//self.num_modality], self.layer_speakerEmb(speakerEmb), ps, gender), dim=1))
            #x_audio = self.layer012(torch.cat((x[:, :feature_dim//self.num_modality],speakerEmb, self.layer_ps(ps)), dim=1)) #self.layer_spfAudio(c)
            #x_audio = self.layer012(torch.cat((x[:, :feature_dim//self.num_modality], ps), dim=1))
            x = x_visual + x_audio

        predictGender = self.guessGender(x_audio)
