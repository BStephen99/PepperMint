...
class SPELL(Module):
    def __init__(self, cfg):
        ...
        if self.use_spf:
            ...
            self.layer_pers = Linear(-1, cfg['proj_dim']+100) # projection person speaking
    ...
    def forward(self, x, edge_index, edge_attr, c=None, ps=None, pers=None):
        ...
        if self.use_spf:
            ...
            #x_visual = self.layer011(torch.cat((x[:, :feature_dim//self.num_modality], self.layer_spf(c), self.layer_ps(ps), self.layer_pers(pers)), dim=1))
        else:
        ...
        if self.num_modality == 1:
            ...
            #x_audio = self.layer012(torch.cat(x[:, :feature_dim//self.num_modality], self.layer_ps(ps)), dim=1)
            #print(ps.shape)
            #print(x[:, :feature_dim//self.num_modality].shape)
            x_audio = self.layer012(torch.cat((x[:, :feature_dim//self.num_modality], ps), dim=1))
            #x_audio = self.layer012(torch.cat((x[:, :feature_dim//self.num_modality], ps, self.layer_ps(pers)), dim=1))
            x = x_visual + x_audio
